...
verifyBlock genesisConfig verifyAllIsKnown (ComponentBlockMain header payload) = do
    lastAdopted <- do
        ...
        case initialEra of
            OBFT _ -> do
                logDebug $ "usVerifyBlocks OBFT: Almost there!!!"
                let epochIndex = header ^. epochIndexL
                    ...
                    BlockHeaderMain mb ->
                        if mb ^. epochIndexL == (header ^. epochIndexL) - 1
                            then do
                                ...
                                logDebug $ "usVerifyBlocks OBFT 2: Almost there!!!"
                                let epochIndex = header ^. epochIndexL
                                whenEpochBoundaryObft epochIndex tipHeader $ \ei -> do
                                    logDebug $ "usVerifyBlocks OBFT 2: We're on epoch boundary. Running processGenesisBlock"
                                    oldEra <- getConsensusEra
                                    logDebug $ sformat ("usVerifyBlocks: Old era " % shown) oldEra
                                    processGenesisBlock genesisConfig ei
                                    newEra <- getConsensusEra
                                    logDebug $ sformat ("usVerifyBlocks: Adopted era " % shown) newEra
                                getAdoptedBV
